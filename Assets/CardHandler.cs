...
public class CardHandler : Singleton<CardHandler> {
    ...
    public void SetSelection (Card card, Vector3 worldSpaceMousePosition) {
        ...
        //only rotate the card if its individual
        if (selection.ChildCard == null) helper.transform.localRotation = Quaternion.Euler (0, 0, GetPickupRotation ());
    }
